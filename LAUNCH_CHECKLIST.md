...
- [x] Background worker — was confirmed NOT deployed (2026-09-22: `agent_worker_state` had zero documents anywhere), fixed same day by running Remy/Odette's jobs in-process on the web service's own event loop instead of a separate Render service (no plan in use here includes a free/cheap worker instance). See DEPLOY.md's "Background worker" section. Trade-off: agent LLM calls now share the single web instance's resources — fine at current load, revisit (Fly.io ~$2/mo) if that changes.
