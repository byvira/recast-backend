...
def _flag(flag_type: str, severity: str, detail: dict, metric: dict) -> dict:
    return {
        "flag_type": flag_type,
        "detection": "rule",
        "severity": severity,
        "summary_persona": odette_flag_summary(flag_type, detail),
        "detail": detail,
        "metric": metric,
    }


async def evaluate_rules(workspace_id: str) -> list[dict]:
    ...
    storm = await _signal_storm(workspace_id, now)
    if storm:
        flags.append(storm)
...
async def _signal_storm(workspace_id: str, now: datetime) -> dict | None:
