"""Tests for GET /api/v1/analytics/posts including real post content.

PostMetrics (app.pipelines.analytics.base) has no content field at all —
post_id is really the originating piece_id (every analytics fetcher sets
post_id=piece_id) — so the Performance page's "Top Posts" table always
rendered an empty '""' where the post text should be. Fixed by joining
